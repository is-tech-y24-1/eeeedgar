...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(CodeFixProviderI7List)), Shared]
    public class CodeFixProviderI7List : CodeFixProvider
    {
        public sealed override ImmutableArray<string> FixableDiagnosticIds
        {
            get { return ImmutableArray.Create(AnalyzerI7List.DiagnosticId); }
        }
        ...
        private async Task<Document> ChangeNullToEmptyCollectionAsync(Document contextDocument, ReturnStatementSyntax returnStatement, CancellationToken cancellationToken)
        {
            ...
            var returnType = method.ReturnType.DescendantNodesAndSelf().OfType<GenericNameSyntax>().First();

            var newReturnExpression = SyntaxFactory.ObjectCreationExpression
                (
